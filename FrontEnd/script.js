...
const PRODUCTS = [
  ...
  { id:17, model:"iPhone 17 Pro Max", storage:"256 GB", color:"Desert Titanium",  price:69999, condition:"Новий",          year:2025, sim:"eSIM",       accent:"#c8956c" },
  { id:18, model:"iPhone 17 Pro Max", storage:"512 GB", color:"Black Titanium",   price:79999, condition:"Новий",          year:2025, sim:"eSIM",       accent:"#2c2c2e" },
  { id:26, model:"iPhone 17 Pro",     storage:"256 GB", color:"Cosmic Orange",    price:62999, condition:"Новий",          year:2025, sim:"eSIM",       accent:"#c8702a" },
  { id:20, model:"iPhone 17 Air",     storage:"128 GB", color:"Sky Blue",         price:54999, condition:"Новий",          year:2025, sim:"eSIM",       accent:"#7ab8d4" },
  { id:21, model:"iPhone 17",         storage:"128 GB", color:"Black",            price:47999, condition:"Новий",          year:2025, sim:"eSIM",       accent:"#1a1a1a" },
  { id:22, model:"iPhone 17",         storage:"256 GB", color:"Sage",             price:52699, condition:"Новий",          year:2025, sim:"eSIM",       accent:"#7a9a7a" },

  // ── iPhone 16 серія (2024) ──────────────────────────────────
  { id:1,  model:"iPhone 16 Pro Max", storage:"256 GB", color:"Titanium Black",   price:56999, condition:"Новий",          year:2024, sim:"eSIM",       accent:"#ff8a1f" },
  { id:2,  model:"iPhone 16 Pro",     storage:"128 GB", color:"Desert Titanium",  price:49999, condition:"Новий",          year:2024, sim:"eSIM",       accent:"#ffb15a" },
  { id:3,  model:"iPhone 16",         storage:"128 GB", color:"Ultramarine",      price:40999, condition:"Новий",          year:2024, sim:"eSIM",       accent:"#8b5cf6" },

  // ── iPhone 15 серія (2023) ──────────────────────────────────
  { id:4,  model:"iPhone 15 Pro Max", storage:"256 GB", color:"Natural Titanium", price:45999, condition:"Новий",          year:2023, sim:"SIM + eSIM", accent:"#f59e0b" },
  { id:5,  model:"iPhone 15",         storage:"128 GB", color:"Pink",             price:32999, condition:"Новий",          year:2023, sim:"SIM + eSIM", accent:"#ec4899" },

  // ── iPhone 14 серія (2022) ──────────────────────────────────
  { id:6,  model:"iPhone 14",         storage:"128 GB", color:"Midnight",         price:27999, condition:"Новий",          year:2022, sim:"SIM + eSIM", accent:"#0f766e" },
  { id:7,  model:"iPhone 14 Pro",     storage:"128 GB", color:"Space Black",      price:28999, condition:"Вживаний (Б/У)", year:2022, sim:"SIM + eSIM", accent:"#64748b" },
  { id:8,  model:"iPhone 14",         storage:"256 GB", color:"Starlight",        price:24999, condition:"Вживаний (Б/У)", year:2022, sim:"SIM + eSIM", accent:"#94a3b8" },

  // ── iPhone 13 серія (2021) ──────────────────────────────────
  { id:9,  model:"iPhone 13 Pro",     storage:"128 GB", color:"Sierra Blue",      price:22999, condition:"Вживаний (Б/У)", year:2021, sim:"SIM + eSIM", accent:"#3b82f6" },
  { id:10, model:"iPhone 13",         storage:"128 GB", color:"Midnight",         price:26999, condition:"Вживаний (Б/У)", year:2021, sim:"SIM + eSIM", accent:"#334155" },

  // ── iPhone 12 серія (2020) ──────────────────────────────────
  { id:11, model:"iPhone 12 Pro",     storage:"128 GB", color:"Pacific Blue",     price:16999, condition:"Вживаний (Б/У)", year:2020, sim:"SIM + eSIM", accent:"#0ea5e9" },
  { id:12, model:"iPhone 12",         storage:"64 GB",  color:"Black",            price:14999, condition:"Вживаний (Б/У)", year:2020, sim:"SIM + eSIM", accent:"#52525b" },

  // ── Інші моделі ─────────────────────────────────────────────
  { id:13, model:"iPhone 11",         storage:"64 GB",  color:"Purple",           price:12699, condition:"Вживаний (Б/У)", year:2026, sim:"SIM + eSIM", accent:"#a855f7" },
  { id:14, model:"iPhone SE 2022",    storage:"64 GB",  color:"Red",              price:9999,  condition:"Вживаний (Б/У)", year:2022, sim:"SIM + eSIM", accent:"#ef4444" },
];
...
function bindHeroButtons() {
  document.querySelectorAll('[data-condition-link]').forEach(btn => {
    btn.addEventListener('click', () => { location.href = `catalog.html?status=${btn.dataset.conditionLink}`; });
  });
  document.querySelectorAll('[data-trade-link]').forEach(btn => {
    btn.addEventListener('click', () => { location.href = 'trade-in.html'; });
  });
}
...
function buildCatalog() {
  ...
  modelBtns.forEach(btn => btn.addEventListener('click', () => { state.model = btn.dataset.model; render(); }));

  resetBtn?.addEventListener('click', () => {
    state.status = 'all'; state.search = ''; state.model = 'all';
    search.value = '';
...
const MODEL_STORAGES = {
  ...
  'iPhone 17 Pro':     ['256 GB','512 GB','1 TB','2 TB'],
...
const FAKE_REVIEWS = [
  { stars:5, text:'Відмінний телефон! Швидка доставка, упаковка ціла. Рекомендую магазин.', author:'Олексій К.', date:'12 травня 2025' },
  { stars:5, text:'Замовляв вже вдруге. Все чесно, б/у стан відповідає опису. Дякую!', author:'Марія В.', date:'3 квітня 2025' },
  { stars:4, text:'Гарний апарат, камера супер. Трохи подряпана рамка, але ціна відповідна.', author:'Ігор Р.', date:'18 березня 2025' },
];
const FAKE_QA = [
  ...
  { q:'Як перевірити оригінальність?', a:'Серійний номер перевіряється на сайті Apple. Всі наші телефони — оригінали.' },
];
...
function bindModal() {
  ...
  overlay.addEventListener('click', e => {
    ...
    if (!btn) return;
    if (btn.dataset.sim)     { modalState.selectedSim     = btn.dataset.sim; }
    if (btn.dataset.color)   { modalState.selectedColor   = btn.dataset.color; }
    if (btn.dataset.storage) { modalState.selectedStorage = btn.dataset.storage; }
  ...
  document.getElementById('modalSubmitBtn').addEventListener('click', () => {
    ...
    if (!phone) { document.getElementById('modalPhone').focus(); return; }
